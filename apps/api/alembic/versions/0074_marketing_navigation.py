"""add contact marketing navigation and rename group marketing

Revision ID: 0074_marketing_navigation
Revises: 0073_account_retirement
...
revision = "0074_marketing_navigation"
down_revision = "0073_account_retirement"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    connection.execute(
        menus.update()
        .where(menus.c.public_id == "menu_marketing_group")
        .values(name="群组营销")
    )

    menu_id = connection.execute(
        sa.select(menus.c.id).where(
            menus.c.public_id == "menu_marketing_contact"
        )
    ).scalar_one_or_none()
    id_tables = _id_tables(connection)
    if menu_id is None:
        marketing_id = connection.execute(
            sa.select(menus.c.id).where(
                menus.c.public_id == "menu_marketing"
            )
        ).scalar_one()
        ...
        connection.execute(
            menus.insert().values(
                id=menu_id,
                public_id="menu_marketing_contact",
                parent_id=marketing_id,
                name="好友营销",
                menu_type="page",
                route_path="/contact-marketing",
                icon="BookUser",
                permission_key="marketing.contact_marketing.read",
                sort_order=240,
                enabled=True,
                visible=True,
                is_builtin=True,
...
def downgrade() -> None:
    ...
    if menu_id is not None:
        connection.execute(
            role_menus.delete().where(role_menus.c.menu_id == menu_id)
        )
