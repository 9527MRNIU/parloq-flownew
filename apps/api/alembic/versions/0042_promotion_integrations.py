...
Revision ID: 0042_promotion_integrations
...
revision = "0042_promotion_integrations"
...
def upgrade() -> None:
    ...
    maximum_sort = connection.execute(
        sa.select(sa.func.max(menus.c.sort_order)).where(menus.c.parent_id == parent_id)
    ).scalar_one_or_none() or 0
    connection.execute(
        menus.update()
        .where(menus.c.public_id == "menu_promotion_channels")
        .values(sort_order=maximum_sort + 1)
    )
    connection.execute(
        menus.update()
        .where(menus.c.public_id == "menu_promotion_domains")
        .values(sort_order=maximum_sort + 2)
    )
