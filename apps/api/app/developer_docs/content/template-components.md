...
新视觉主题应声明 `account-link-elements/v1`，并组合平台提供的 Web Components：
...
组件拥有配对网络行为和关键状态逻辑。主题通过 CSS 变量和公开的 `::part()` 调整外观，不应复制或替换组件内部请求代码。
...
平台能力文案内置支持 `en`、`zh-CN`、`hi`、`id`、`pt-BR`、`es`、`ru`、`ur`、`de`、`tr`、`ar`、`fa`、`bn`、`it` 和 `fr`。模板自己的营销文案仍需在每个声明语言中完整提供。
