...
| 群组同步 `groupDetails` | `groupFetchAllParticipating()` | `groupCount`、群 ID、名称、人数、权限、跨群去重成员数 | 是，明细已落主系统资源表 | 中，需要读取全部参与群和成员身份 | 开 | 已删除 `groupSummary`；服务概览、评分和后续群组营销 |
...
产品开关之间不再建立历史同步依赖。开启“好友同步” `contacts` 后，网关只在显式资料同步任务中临时启用相应历史同步配置，日常连接和发信重连不请求完整历史；“群组同步”通过独立群组接口获取数据，不依赖历史同步。
