...
- 模板 ZIP 不得自行加入第三方统计、隐藏 iframe、指纹采集或外部脚本；平台可通过集成管理按模板统一注入已经上传 ZIP 资源包并绑定源域名的 iframe、JavaScript、设备识别与匿名关联组件。
...
- 模板自行携带的外部 JavaScript、未知第三方 SDK、隐藏 iframe 或跨域数据回传；通过集成管理上传 ZIP 资源包、绑定已验证源域名并由平台渲染管线统一注入的组件除外；
