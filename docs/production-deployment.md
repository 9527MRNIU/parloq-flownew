...
脚本在 SSH 交互终端中会打印主机、端口、数据库、用户名、原始密码和可直接复制的
完整连接串，连接串内的密码已自动完成 URL 编码。通过宝塔后台任务、管道或重定向
等非交互方式运行时只显示密码占位符，避免密码进入任务日志。打开前必须确认
PostgreSQL 与 Redis 均为 healthy，且生产 `.env` 已配置两项密码。
