...
好友同步开启后，网关会临时扫描历史包里的聊天对象和消息元数据，把已保存联系人和有过一对一联系的对象合并为好友。处理完成后丢弃聊天列表和消息正文。
...
| 去重群成员数 | 全部群的 `participants` | 排除本账号，按 JID/LID 映射归一后跨群去重，用于评分 |

第一期不落库完整群成员列表。网关在一次群组同步中临时汇总全部 `participants`，排除本账号，优先按手机号 JID、其次按已知 LID/JID 映射、最后按原始 ID 去重，只把 `uniqueGroupMemberCount` 聚合值写回主系统。群详情仍只保存计算后的 `ownRole` 和 `canSend`，避免为了评分增加群成员表。未来真的需要成员筛选或成员营销时，再增加群成员表。
...
- **有过联系的对象**：存在至少一条一对一历史或实时消息，即使没有保存过也计入好友；
- **群组、广播和系统对象**：排除，不进入联系人资源。

业务上的“好友”是已保存联系人和有过一对一联系的对象取并集并去重，群组、广播和系统对象除外。即使存在 `Contact.name`，也只能称为“Baileys 已同步的已保存联系人”，不能保证覆盖手机中的完整通讯录，也不能证明对方保存了本账号。
...
- `unique_group_member_count`：全部有效群排除本账号后，按身份归一并跨群去重的成员数，用于评分。

`friend_count` 改为有效的已保存联系人与有过一对一联系的对象并集去重数，即 `is_saved_contact=true OR has_chat_history=true`。只有 `notify`（对方自己设置的显示名），但既没有保存联系人证据、也没有一对一消息记录的对象，不计入好友数。
...
2. **好友**：展示已保存联系人和有过一对一联系的对象，支持按来源、名称、号码搜索，并展示 JID/LID 和最后同步时间；
...
| 好友 | 每个好友加 1 分 | 已保存联系人与有过一对一联系的对象并集去重数 |
| 群成员 | 每 5 个去重后的外部群成员加 1 分，向下取整 | 全部有效群的成员身份归一、排除本账号并跨群去重后的数量 |
...
账户评分 = (有头像 ? 5 : 0)
         + friend_count
         + floor(unique_group_member_count / 5)
```
