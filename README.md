...
脚本只提供“状态、打开、关闭”三个菜单。打开时从 `20000-29999` 选择两个未占用的
随机 TCP 端口，通过宝塔自己的防火墙管理入口把规则登记到“宝塔 → 安全”，再转发
...
公网打开后不限制来源 IP，PostgreSQL 与 Redis 均依赖服务密码认证。全新部署时，
标准发布脚本会同时生成独立的 `POSTGRES_PASSWORD` 和 `REDIS_PASSWORD` 并以 `600`
权限保存到生产 `.env`，不会输出到终端。已有环境缺少 Redis 密码时发布脚本会直接
中止，必须由管理员一次性人工补齐。公网开关脚本在 SSH 交互终端中会读取并打印
PostgreSQL、Redis 的原始密码和已完成 URL 编码的完整连接串；通过后台任务、管道或
重定向等非交互方式运行时仍只打印密码占位符，避免密码进入任务日志。
